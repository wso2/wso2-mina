 ...
 *
 */
package org.wso2.org.apache.mina.filter.support;
...
public class SSLHandler {
    ...
    public void destroy() {
        ...
        try {
            ...
            SessionLog.debug(session, "Unexpected exception from SSLEngine.closeInbound().", e);
        }
        ...
        try {
            ...
            SessionLog.debug(session, "Unexpected exception from SSLEngine.wrap().", e);
        }
    ...
    public void flushPreHandshakeEvents() throws SSLException {
        ...
        while ((scheduledWrite = preHandshakeEventQueue.poll()) != null) {
            ...
            parent.filterWrite(scheduledWrite.nextFilter, session, (IoFilter.WriteRequest) scheduledWrite.data);
        }
    }

    public void scheduleFilterWrite(IoFilter.NextFilter nextFilter, IoFilter.WriteRequest writeRequest) {
        filterWriteEventQueue.offer(new Event(EventType.FILTER_WRITE, nextFilter, writeRequest));
    }

    public void scheduleMessageReceived(IoFilter.NextFilter nextFilter, Object message) {
        messageReceivedEventQueue.offer(new Event(EventType.RECEIVED, nextFilter, message));
    }
            ...
            try {
                do {
                    ...
                    while ((e = filterWriteEventQueue.poll()) != null) {
                        e.nextFilter.filterWrite(session, (IoFilter.WriteRequest) e.data);
                    }
    ...
    public void messageReceived(IoFilter.NextFilter nextFilter, ByteBuffer buf) throws SSLException {
        if (buf.limit() > inNetBuffer.remaining()) {
            ...
            inNetBuffer = SSLByteBufferPool.expandBuffer(inNetBuffer, inNetBuffer.capacity() + buf.limit() * 2);
            ...
            if (SessionLog.isDebugEnabled(session)) {
                SessionLog.debug(session, " expanded inNetBuffer:" + inNetBuffer);
        ...
        while (src.hasRemaining()) {
            ...
            if (result.getStatus() == SSLEngineResult.Status.OK) {
                ...
                throw new SSLException(
                        "SSLEngine error during encrypt: " + result.getStatus() + " src: " + src + "outNetBuffer: "
                                + outNetBuffer);
            }
        }
    ...
    private void checkStatus(SSLEngineResult res) throws SSLException {
        ...
        if (status != SSLEngineResult.Status.OK && status != SSLEngineResult.Status.CLOSED
            ...
            throw new SSLException(
                    "SSLEngine error during decrypt: " + status + " inNetBuffer: " + inNetBuffer + "appBuffer: "
                            + appBuffer);
        }
    }
    ...
    public WriteFuture writeNetBuffer(IoFilter.NextFilter nextFilter) throws SSLException {
        ...
        try {
            ...
            org.wso2.org.apache.mina.common.ByteBuffer writeBuffer = copy(getOutNetBuffer());
            if (SessionLog.isDebugEnabled(session)) {
            ...
            parent.filterWrite(nextFilter, session, new IoFilter.WriteRequest(writeBuffer, writeFuture));
            ...
            while (needToCompleteHandshake()) {
                ...
                if (getOutNetBuffer().hasRemaining()) {
                    ...
                    org.wso2.org.apache.mina.common.ByteBuffer writeBuffer2 = copy(getOutNetBuffer());
                    writeFuture = new DefaultWriteFuture(session);
                    parent.filterWrite(nextFilter, session, new IoFilter.WriteRequest(writeBuffer2, writeFuture));
                }
            }
        } finally {
            writingEncryptedData = false;
    ...
    private SSLEngineResult.HandshakeStatus doTasks() {
        ...
        if (SessionLog.isDebugEnabled(session)) {
            SessionLog.debug(session, "   doTasks(): " + sslEngine.getHandshakeStatus());
        }
     ...
     */
    public static org.wso2.org.apache.mina.common.ByteBuffer copy(java.nio.ByteBuffer src) {
        org.wso2.org.apache.mina.common.ByteBuffer copy = org.wso2.org.apache.mina.common.ByteBuffer.allocate(src.remaining());
        copy.put(src);
        copy.flip();
        return copy;
    }
